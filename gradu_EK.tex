...
Agentin opettaminen aloitetaan komentoriviltä komennolla mlagents-learn (polku konfiguraatio -tiedostoon) --run-id=(opetussession nimi). Komentorivi näyttää käyttäjän asettamien opetusaskelten välein mm. agentin keskiarvopalkkion ja keskihajonnan(std of reward?) askelten aikana. Tässä tutkimuksessa asetettiin väliksi 10000, tensorboard päivittyy myös 10000 välein ts. piirtää kuvaajan pisteen?

\section{Agentin havainnot ja palkkiot}

Koneoppimisagentin oppimista ohjaavat sen tekemät havainnot ja sen saamat palkkiot. Havaintoja voi tallentaa suoraan skriptitiedostossa käsin \textit{AddObservation()}-metodilla, jolle annetaan parametrina datatyyppi ja sen arvo. Tämän tutkimuksen reitinhakutehtävissä agentti havainnoi omaa ja kohteen sijaintia kaksiulotteisessa vektorimuodossa \textit{(x,y)}, jolloin jokaisella askeleella neuroverkko saa tiedon näistä sijainneista. Tähän havaintoon käytetään Unityn omaa datatyyppiä \textit{Vector2}, joka sisältää \(x\) ja \(y\) arvot \textit{float}-tyyppisinä. Lisäksi agentille lisättiin kaksi \textit{RayPerceptionSensor2D}-komponenttia, jotka lähettävät agentista osumatietoja kerääviä säteitä. Ensimmäinen sensori sisältää kahdeksan sädettä, jotka jakautuvat tasaisesti agentin ympärille. Säteet ovat pituudeltaan kaksi yksikköä ja tunnistavat ainoastaan pelialueen esteitä. Tämä määritellään erillisen tagin avulla eli tässä tapauksessa esteillä on Collision-tagi ja säteet tunnistavat ainoastaan kyseisen tagin peliobjektit. Toinen sensori lähettää myös kahdeksan sädettä, jotka ovat 20 yksikköä pitkiä. Nämä säteet tunnistavat ainoastaan kohteen, joka on merkitty End-tagilla. Sensorit lähettävät automaattisesti datan \textit{float}-taulukkona neuroverkolle, joten sitä ei tarvitse itse toteuttaa skriptitiedostossa.

Agentin saamat palkkiot ovat vahvistusoppimisen ydin ja tässä tapauksessa agenttia palkitaan sen nopeuden ja suoriutumisen perusteella. Jokaisen agentin päätöksen yhteydessä annetaan pieni negatiivinen palkkio, jolloin agenttia ohjataan suorittamaan tehtävä niin nopeasti kuin mahdollista. Palkkio määritellään skriptitiedostossa \textit{AddReward(-1/MaxStep)}, jossa \textit{MaxStep} on yhden opetusjakson maksimipituus eli kuinka monta päätöstä agentti voi yhden jakson aikana tehdä. Tässä tutkimuksessa pelialueet ovat melko pieniä, joten maksimipituudeksi asetettiin 500. Jos agentti ei saa suoritettua tehtävää, se saa jaksosta kokonaispalkkioksi \[-1/MaxStep * MaxStep\] eli -1. Jos agentti osuu kohteeseen, sille annetaan palkkioksi +1 eli \textit{AddReward(1)}. Jos agentti osuu matkalla esteeseen, sille annetaan palkkioksi -1 ja jakso lopetetaan ja agentin sijainti asetetaan oletuksena jakson alkupisteeseen.
...
\begin{figure}[h]
%\centering
\includegraphics[width=7cm]{Area4-cumulative-reward-sac.png}
\caption{TESTIKUVA: Alueen 4 kumulatiivinen palkkio 2 miljoonan opetusaskeleen aikana Soft Actor Critic -algoritmilla.}
\label{area4cumulativerewardsac}
\end{figure}

\begin{figure}[h]
%\centering
\includegraphics[width=7cm]{Area4-policy-loss-sac.png}
\caption{TESTIKUVA: Alueen 4 käytännön menetys 2 miljoonan opetusaskeleen aikana Soft Actor Critic -algoritmilla.}
\label{area4policylosssac}
\end{figure}

\begin{figure}[h]
%\centering
\includegraphics[width=7cm]{Area4-value-loss-sac.png}
\caption{TESTIKUVA: Alueen 4 arvon menetys 2 miljoonan opetusaskeleen aikana Soft Actor Critic -algoritmilla.}
\label{area4valuelosssac}
\end{figure}
