...
Neuroverkkojen perusosiin kuuluvat perseptronit (engl. perceptron), jotka ovat keinotekoisia neuroneita \parencite{nielsen2015neural}. Perseptronit koostuvat binäärisistä syötteistä \(x_{1}, x_{2}, x_{3},..,\) ja yksittäisestä binäärisestä tuloksesta. Syötteiden relevanttius määritellään erillisten painoarvojen (engl. weight) \(w_{1}, w_{2}, w_{3},..,\) perusteella. Syötteet ja painoarvot lasketaan painotettuna summana $\sum_{j}w_{j}x_{j}$ ja tulokseen lisätään neuronin kynnysarvo (engl. threshold tai bias), joka on neuronille parametrina annettu luku ja määrittää onko lopullisena tuloksena 0 vai 1. Yksinkertaistettuna \cite{nielsen2015neural} kuvaa perseptronia "laitteeksi, joka tekee päätökset punnitsemalla todisteita".
...
Oppiminen on mahdollista painoarvoja muuttamalla, jolloin koko neuroverkon lopullinen tulos muuttuu \parencite{nielsen2015neural}. Vaarana kuitenkin on, että pienet muutokset muuttavat neuroverkon tulosta radikaalisti vastakkaiseen suuntaan. Korvaamalla alkuperäiset, binääriset perseptronit Sigmoid-neuroneilla painoarvojen muutokset eivät aiheuta suurta muutosta tuloksessa. Sigmoid-neuronit käyttävät sigmoid-funktiota porrasfunktion sijaan, jolloin tulos voi olla mitä tahansa 0 ja 1 välillä. Sigmoid-funktion matemaattinen kaava on \(\frac{1}{1+e^{-1}}\).

Kuten koneoppimisessa, myös syväoppimisessa neuroverkon opettaminen tapahtuu harjoitusdatan avulla \parencite{nielsen2015neural}. Luokitteluongelman suoran ratkaisemisen sijaan neuroverkon opettamisessa keskitytään ensin painoarvojen ja kynnysarvojen muodostaman virhefunktion minimoimiseen ja vasta sen jälkeen luokittelun tarkkuuteen. Virhefunktion minimoimiseen käytetään tunnettua optimointialgoritmia, gradienttimenetelmää (engl. gradient descent). Gradienttimenetelmällä pyritään selvittämään virhefunktion globaali minimi valitsemalla aluksi satunnaisesti yksittäinen piste funktiosta ja suorittamalla pieniä askeleita kohti globaalia minimiä. Askeleiden pituus määritellään oppimisnopeudella (engl. learning rate), joka on erikseen sovittu vakioluku. Oppimisnopeuden on tarkoitus olla melko pieni luku, jottei askel hyppää globaalin minimin yli. Toisaalta se ei saa olla liian pieni, koska muuten algoritmi toimii hyvin hitaasti.
