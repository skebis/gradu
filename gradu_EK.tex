...
A*-algoritmin heikkous on huono skaalautuvuus suurien alueiden reitinhakuun. A*-algoritmi ei myöskään pysty suoriutumaan tuntemattoman ja muuttuvan alueen reitinhausta, koska algoritmin toiminta estyy esteen tullessa lasketulle polulle. Ongelmat esiintyvät videopelien lisäksi etenkin robotiikassa ja ne ovat inspiroineet tutkijoita monien eri variaatioiden kehittämiseen \parencite{duchovn2014path}.

D* (Dynamic A*) on A*-algoritmin variaatio, joka pyrkii ottamaan huomioon alueen muutokset reitinhaun aikana. (Stentz 1994, optimal and efficient path planning for partially known environments). Focussed D* algoritmi on myös Stentzin kehittämä variaatio, jonka ideana on vähentää D*-algoritmin laskennallisia vaatimuksia (Stentz 1995, The Focussed D* Algorithm for Real-Time Replanning). Lopulta Koenig (2005) esitteli D* Lite -algoritmin, joka nimestään huolimatta ei varsinaisesti perustu suoraan D*-algoritmiin, vaan A*- ja LPA*-algoritmiin (Koenig 2005, Fast Replanning for Navigation in Unknown Terrain).
