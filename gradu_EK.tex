...
Koneoppimisen tutkimus voidaan jakaa ohjattuun oppimiseen, ohjaamattomaan oppimiseen ja vahvistusoppimiseen. Ohjatussa oppimisessa oppiminen tapahtuu luokitellun aineiston perusteella. Se havainnollistaa lineaariapproksimaatiota ja siihen liittyviä ongelmia. Opetusaineisto tuodaan \((x,y)\) pareina, jossa \(x\) on syöte, esimerkiksi kuva, ja \(y\) on tulos, esimerkiksi lintu tai todennäköisyysjakauma. Oppijan tavoitteena on tuottaa ennuste \(y\) syötteen \(x\) perusteella. Ohjatun oppimisen heikkoutena voidaan pitää luokitellun datan saatavuutta. Data täytyy esikäsitellä ja luokitella ennalta muilla menetelmillä, joka nostaa lopullista kustannusta. \parencite{das2017survey}.

Ohjaamattomassa oppimisessa analysoidaan luokittelematonta dataa. Tarkoituksena on tehdä päätelmiä datan rakenteesta ja malleista ilman ihmisen väliintuloa ja ohjeistusta. Klusteroinniksi kutsutaan menetelmiä, joissa datapisteet jaetaan ominaispiirteidensä perusteella klustereihin. Yksittäiset klusterit sisältävät siis ominaisuuksiltaan samankaltaisen datan. \textit{K-means} -klusterointi on tunnettu klusterointimenetelmä, jossa datapisteet \(n\) jaetaan \(K\):hon klusteriin. Klusterin keskipistettä lähimpänä oleva datapiste toimii prototyyppinä, johon muiden datapisteiden etäisyyksiä verrataan. \textit{Dimensionality reduction}-menetelmät pyrkivät tihentämään datajoukkoa ja sen analysointia. Sen periaatteena on vähentää datajoukossa tarkasteltavia ominaispiirteitä ja näin ollen vähentää datajoukon ulottuvuutta, jolloin sen analysointi helpottuu. Uusi datajoukko pyritään pitämään samankaltaisena, jottei analysointiin tulisi muutoksia.

TODO: tähän vahvistusoppiminen, koska yksi paradigmoista. paradigma oikea termi näille "ala-lajeille"? alaotsikointi vielä kuntoon, mahdollisesti ala-ala-otsikoita mukaan.
