...
Reitinhaulla tarkoitetaan yksinkertaisimmillaan reitin selvittämistä kahden pisteen välillä. Se on yksi videopelien tekoälyn ja myös robotiikan tunnetuimmista ja haastavimmista ongelmista, jota on tutkittu jo muutaman vuosikymmenen ajan \parencite{cui2011based,abd2015comprehensive}. Reitinhakua esiintyy monissa eri peligenreissä, kuten roolipeleissä ja reaaliaikaisissa strategiapeleissä, joissa ei-pelaaja-hahmoja (engl. non-player-character, NPC) määrätään liikkumaan ennaltamäärättyyn tai pelaajan määräämään sijaintiin väistellen samalla vastaantulevia esteitä. Suurimpia haasteita ovatkin esteiden järkevä väistäminen, tehokkaimman reitin löytäminen ja suoritustehon optimoiminen. \parencite{abd2015comprehensive,cui2011based}. Suoritustehoon liittyen etenkin muistinkäyttöä ja laskentatehoa pidetään yleisesti rajoittavina tekijöinä videopelien reitinhaussa \parencite{botea2013pathfinding}. Suoritustehoon liittyviä ongelmia voidaan ratkaista erilaisilla algoritmeilla tai pelialueen esitystapaan liittyvillä ratkaisuilla \parencite{botea2013pathfinding,cui2011based}.
...
Reitinhaun on oltava videopeleissä nopeaa, laskennallisesti tehokasta ja realistista, jotta pelaaja pystyy syventymään videopelin maailmaan eikä pelaajan kokema immersio keskeydy. Eri videopeleissä ilmenee usein erilaisia reitinhakuongelmia, joita ei mikään yksittäinen reitinhakualgoritmi pysty ratkaisemaan. Esimerkiksi reaaliaikaisissa strategiapeleissä pelaaja voi kontrolloida kymmeniä ellei jopa satoja hahmoja samanaikaisesti. Toisaalta roolipeleissä kaupungit ovat täynnä ei-pelaaja-hahmoja, jotka mahdollisesti vaeltelevat reitinhakualgoritmien avulla ja väistelevät vastaantulevia esteitä. Erityisesti kaupungeissa ajavat autot joutuvat reagoimaan pelaajan liikkeisiin tieosuuksilla.

Suurimmat haasteet ovat vuosikymmenten aikana olleet suoritustehoihin liittyviä. Vaikka komponentit ovatkin kehittyneet vuosi vuodelta nopeasti, myös videopelit ovat monimutkaistuneet ja niiden laskennalliset vaatimukset kasvaneet. Reitinhakualgoritmeille varatut resurssit ovat videopeleissä rajatut, koska resursseja käytetään hyvin paljon esimerkiksi graafisiin ja fysikaalisiin ominaisuuksiin. TODO: tästä eteenpäin tarkistus ettei toistoa

Yhden agentin staattisen ruudukkoalueen reitinhakuongelma on ratkaistavissa optimaalisesti heuristisilla reitinhakualgoritmeilla, mutta nykyään videopeleissä reitinhakuongelmat ovat monimutkaisempia ja saattavat vaatia useiden eri kriteerien täyttymisen. Reitinhakuongelmat pitävät sisällään esimerkiksi useamman agentin samanaikaista reitinhakua, reaaliajassa muuttuvien alueiden reitinhakua tai tuntemattomien alueiden reitinhakua. Lisäksi reitinhakualgoritmin täytyy minimoida suoritustehon ja tallennustilan käyttö. Nykytutkimus keskittyykin pääasiassa monimutkaisiin reitinhakuongelmiin ja algoritmien vertailuun eri tilanteissa.
...
Vahvistusoppimisen malli voidaan kuvata tilojen \(S\), toimintojen \(A\) ja palkkiosignaalien \(r\) avulla. Jokaisella aika-askeleella \(t\) agentti suorittaa toiminnan \(a\), jolloin ympäristön tila \(s\) muuttuu ja muutos viestitään agentille palkkiosignaalin \(r\) kautta. Palkkiosignaalin lisäksi ympäristö viestii agentille tilamuutoksesta, jolloin agentti saa tiedon uudesta tilasta \(s+1\). Palkkion ja uuden tilan perusteella agentti valitsee jälleen seuraavan toimintonsa. Agentin lopullisena tavoitteena on saavuttaa paras mahdollinen käytäntö (engl. policy), joka maksimoi saadun palkkion määrän. \parencite{arulkumaran2017brief}. Kuvio \ref{reinflearning} havainnollistaa vahvistusoppimisen toimintaa. TODO: kuvion käyttäminen järkevämmin?
